...
class DoGPyramid:
    ...
    def __compute_blurred_images(self):
        ...
        filter_size = int(2 * np.ceil(3 * sigma) + 1.0)  # $w\approx 2\times\lceil 3\sigma\rceil + 1$
        ...
        for octave_idx in range(self.num_octaves):
            ...
            for level_idx in range(1, num_images_per_oct):
                sigma = self.sigma * sigma_factor * (self.k ** (level_idx - 1))  # $\sigma * \sqrt{k^2-1} * k^{l-1}$

                filter_size = int(2 * np.ceil(3 * sigma) + 1.0)
                octave_stack[:, :, level_idx] = cv2.GaussianBlur(octave_stack[:, :, level_idx - 1],
                                                                 (filter_size, filter_size), sigma)
    ...
    def __compute_difference_of_gaussians(self, blurred_images):
        ...
        for idx, _img in enumerate(blurred_images):
            ...
            for dog_idx in range(num_dogs_per_octave):
                dog[:, :, dog_idx] = np.abs(_img[:, :, dog_idx + 1] - _img[:, :, dog_idx])
    ...
    def __extrema_detection(self, diff_of_gaussians):
        ...
        for oct_idx, dog in enumerate(diff_of_gaussians):
            ...
            is_keypoint = (dog == dog_max) & (dog >= self.t)  # non-maximum suppression
            is_keypoint[:, :, 0] = False
            is_keypoint[:, :, -1] = False
...
if __name__ == '__main__':
    ...
    im_with_keypoints = cv2.drawKeypoints(img_scaled, kp, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
