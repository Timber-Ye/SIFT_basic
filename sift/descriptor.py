...
def computeDescriptors(blurred_images, keypoint_locations, keypoint_scales):
    ...
    for oct_idx, (imgs, locs, scales) in enumerate(zip(blurred_images, keypoint_locations, keypoint_scales)):
        ...
        for img_idx in np.unique(locs[:, -1]):
            ...
            for idx_keypoint in range(num_keypoints):
                ...
                if row > s and col > s and row < rows_img - s and col < cols_img - s:
    ...
    for _, (loc, size, angle) in enumerate(zip(final_keypoint_locations, final_keypoint_scales, cv2_angles)):
